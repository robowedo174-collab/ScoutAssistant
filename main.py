...
SYSTEM_PROMPT_BASE = """
Твоё имя — **Андрей Куракин младший**. Ты — эксперт по скаутской педагогике.
...
3.  **Вариативность:** Предложи Вариант А (попроще) и Вариант В (посложнее/другой формат).
...
async def set_main_menu(bot: Bot):
    ...
    """
    main_menu_commands = [
        types.BotCommand(command='/show_goal', description='Показать текущую Цель программы 🎯'),
        types.BotCommand(command='/set_goal', description='Изменить/установить новую Цель 📝')
    ]
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if current_goal:
        ...
        await message.answer(
            f"Я — **Андрей Куракин младший**. Без Цели мы никуда. \n"
